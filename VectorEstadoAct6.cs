...
namespace TP4_Final
{
	public class VectorEstadoAct6
	{
		...
		public VectorEstadoAct6(Dictionary<int, Distribucion> distribuciones)
		{
		...
		public VectorEstadoAct6(VectorEstadoAct6 ve, Dictionary<int, Distribucion> distribuciones, bool keep = false)
		{
			Evento = ve.Evento;
			Reloj = ve.Reloj;
		...
		private Servicio CopiaServicio(Servicio s)
		{
			...
			c.ClienteEnAtencion = new Cliente[s.Empleados.Count];
			for (int i = 0; i < c.ClienteEnAtencion.Length; i++)
				if (s.ClienteEnAtencion[i] != null)
					c.ClienteEnAtencion[i] = new Cliente(s.ClienteEnAtencion[i]);
